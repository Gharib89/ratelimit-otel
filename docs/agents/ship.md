...
- The **release archive** (ADR-0006): the asset names `ratelimit-otel-<version>.zip` and `ratelimit-otel-<version>.zip.sha256`, the download URL `https://github.com/Gharib89/ratelimit-otel/releases/download/ratelimit-otel--v<version>/ratelimit-otel-<version>.zip`, the zip's layout (`.claude-plugin/plugin.json` and `hooks/` at its root) and the sha256 line in the release notes. The operator builds the console entry from these, so a rename breaks the next rollout.
