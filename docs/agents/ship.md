...
Run: `claude -p "<prompt>" --plugin-dir plugin --debug-file <log> < /dev/null`, then read the result back **at the collector**: query cc-otel's `raw.metrics` for rows under scope `cc-otel.plugin` newer than the run and assert both contracted metric names, `claude_code.usage.utilization` and `claude_code.usage.reset_in_seconds`, carrying the `window` and `resets_at` datapoint attributes ADR-0001 fixes. The repo's `CLAUDE.md`, under **Verifying a delivery landed**, carries the read-only query and the `scope_name` filter that separates these rows from the retiring wrapper's under the same two names. `<log>` is a supporting read, never the proof: its `$.http.fetch (ratelimit-otel): POST <endpoint>/v1/metrics` line and a 2xx say a request was accepted, and the collector attests every record and silently drops a foreign one (cc-otel ADR-0030), so a green POST with no row is the failure this entry exists to catch. `-p --debug` alone prints nothing, and without `< /dev/null` the run waits on stdin. To read the payload itself, run a copy of the plugin that logs it before the POST. A copy shares the store file, so clear the delivery floor first by deleting `~/.claude/plugins/store/ratelimit-otel_inline-*.json`, or the floor answers instead of the collector.
Needs: **read access to cc-otel's `raw.metrics`**, and a seat the **console telemetry env block** reaches, which is what supplies the endpoint. Probe the read access **before** running anything, with the TCP probe in the repo's `CLAUDE.md` under **Verifying a delivery landed**: no route to the database is silent rather than an error, and a runner that queries anyway reads an empty result as "nothing was sent" and reports a pass it did not earn. Blocked means the full-tunnel VPN or a closed pinhole, and that file names both ways round it. The seat is read off `<log>` after the run, by asserting a `POST` line, since a seat the block does not reach logs nothing at all.
Without it: hand-off
Also proven by CI: None.
Claims to probe: that a POST from inside a hook returns 2xx; that `$.http.fetch` is not blocked here by the `allow_web_fetch` policy or by session-level nonessential-traffic disablement. Already measured on a fleet seat, 2026-09-21, and not to be re-asked: the console telemetry env block beats both a process-env value set on the `claude -p` invocation and an `env` block in a `--settings` file, so no override route points this plugin at a local listener; and the OTEL names reach `$.env.get` inside a hook but are not exported to child processes, so `env | grep OTEL` in a Bash child reads a false absence.
...
Needs: `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS` truthy from a source that outranks process env, which the 2026-09-21 measurement recorded under the entry above settles: the console telemetry env block carries it alongside the OTEL names and beat both process env and a `--settings` `env` block, so process env cannot supply it on a seat that block reaches. Unlike the OTEL names it does reach child processes. Detect by asserting the hook ran at all, because its absence is a silent no-op and not an error.
Without it: hand-off
Also proven by CI: None.
