...
- **Nothing Claude Code loads as project context sits at the repo root.** The repo root is the plugin root, so `claude plugin validate . --strict` fails on a `CLAUDE.md` there: `root: CLAUDE.md at the plugin root is not loaded as project context`. The project's instructions live at `.claude/CLAUDE.md`, which loads the same way and validates clean.
