...
- **No value from the console env block is written into this repo.** The endpoint host, the bearer carried in `OTEL_EXPORTER_OTLP_HEADERS`, and every other value the org console fans out are read at runtime through `$.env.get` and never committed: not in an ADR, not in a test fixture, not in a comment, not in a commit message. Name the mechanism, never the value. This repo is public and the `secrets` gate is `gitleaks` over the diff, which catches a credential shape and not a hostname, so this rule is the only thing covering the endpoint ([ADR-0004](../adr/0004-transport-reads-the-console-env-block.md)).
